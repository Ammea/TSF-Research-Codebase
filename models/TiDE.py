...
class Model(nn.Module):
    def __init__(
        ...
        self.r = configs.r
        self.H = configs.H
        self.encoder = Encoder(configs.enc_layer_num, configs.hidden_dim, configs.r,
                               configs.r_hat, configs.L, configs.H, configs.feature_projection_dim)
        self.decoder = Decoder(configs.dec_layer_num, configs.hidden_dim,
                               configs.r_hat, configs.H, configs.p, configs.temp_dec_dim)
        self.residual = nn.Linear(configs.L, configs.H)


    def forward(self, x, batch_y):
        covariates = torch.cat([x[:, :, 5:], batch_y[:, -self.H:, 5:]], dim=1)
        x = x[:, :, 0:5]
        # covariates = batch_y[:, :, 0:2]
        attributes = torch.ones(x.shape[0])
        # attributes = []
        # x: [B,L,N], covariates: [B,L+H,N,r], attributes: [B,N,1]
        batch_size = x.size(0)
        attributes = attributes.unsqueeze(-1).unsqueeze(-1).repeat((1, x.size(2), 1))
        a = covariates.unsqueeze(-2)
        c = covariates.unsqueeze(-2).repeat((1, 1, x.size(2), 1))
        # b = covariates.unsqueeze(-2).repeat((1, x.size(2), 1))

        covariates = covariates.unsqueeze(-2).repeat((1, 1, x.size(2), 1))
